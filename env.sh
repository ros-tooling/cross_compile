...
TOOLCHAIN_SCRIPT=$THIS_DIR/cmake-toolchains/$TARGET_ARCHITECTURE.sh
if [ ! -e "$TOOLCHAIN_SCRIPT" ]; then
    ...
    echo "Path to target specific toolchain: $THIS_DIR/cmake-toolchains/$TARGET_ARCHITECTURE.sh"
    return 0
fi